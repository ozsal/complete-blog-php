...
?>
<?php	/*returns all post under a topic*/

	function getPublishedPostsByTopic($topic_id){
		global $conn;
		$sql = "SELECT * FROM posts ps
				WHERE ps.id IN (SELECT pt.post_id FROM post_topic pt
				WHERE pt.topic_id=$topic_id GROUP BY pt.post_id HAVING COUNT(1) = 1)";
		$result = mysqli_query($conn,$sql);

		//fetch all post as an associative array called $posts
		$posts = mysqli_fetch_all($result, MYSQLI_ASSOC);
	...
	function getPost($slug){
		...
		if ($post) {
			//get the topic to which this post belong
			$post['topic'] = getPostTopic($post['id']);
		}
		return $post;
